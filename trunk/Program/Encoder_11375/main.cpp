...
LargeInteger GetX0(string password);
LargeInteger GetMu(string password);
...
int main(int argc, char *argv[])
{
   ...
   // get initial values
   LargeInteger x0 = GetX0(password1);
   LargeInteger mu = GetMu(password1);
   ...
   if(encode == true)
   {
      for(int i = 0; i < (int)scheme.size(); i++)
      {
         ...
         if(scheme[i] == 'e' || scheme[i] == 'E')
            EncryptFile(outputfile, x0, mu, jump);
         else if(scheme[i] == 'd' || scheme[i] == 'D')
            DecryptFile(outputfile, x0, mu, jump);
      }
...
LargeInteger GetX0(string password)
{
   ...
   return LargeInteger(sum) * 100000000 + LargeInteger(sum2);
}
...
LargeInteger GetMu(string password)
{
   ...
   return LargeInteger(4) * 100000000 - product / 8;
}
